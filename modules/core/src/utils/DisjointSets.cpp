...
IndexType DisjointSets<IndexType>::find(IndexType i)
{
    ...
    // This performs path halving, replacing every other parent pointer with its grandparent pointer.
    // It has the same worst-case time complexity as full path compression, but is faster in practice.
