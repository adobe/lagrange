...
struct MDLImpl {
    ...
    MDLImpl() {
        ...
        //Set up compiler 
        state.compiler = handle_of(
            state.sdk->get_api_component<IMdl_compiler>());

        //Load the FreeImage plugin.
        LA_ASSERT(state.compiler->load_plugin_library("nv_freeimage" MI_BASE_DLL_FILE_EXT) == 0, "Loading FreeImage");

        state.sdk->start();

        state.factory = handle_of(
            state.sdk->get_api_component<IMdl_factory>());
        ...
        */
        ...
        auto context = handle_of(state.factory->create_execution_context());
        {
            ...
            mi::Sint32 err_code;            
            err_code = state.compiler->load_module_from_string(state.transaction.get(), "::adobe::annotations", annotations.c_str(), context.get());
            if (err_code < 0) {
                logger().error("Failed to load module ::adobe::annotations, code: {}", err_code);
            }

            err_code = state.compiler->load_module_from_string(state.transaction.get(), "::adobe::convert", convert.c_str(), context.get());
            if (err_code < 0) {
                logger().error("Failed to load module ::adobe::convert, code: {}", err_code);
            }

            err_code = state.compiler->load_module_from_string(state.transaction.get(), "::adobe::mtl", mtl.c_str(), context.get());
            if (err_code < 0) {
                logger().error("Failed to load module ::adobe::mtl, code: {}", err_code);
            }

            err_code = state.compiler->load_module_from_string(state.transaction.get(), "::adobe::util", util.c_str(), context.get());
            if (err_code < 0) {
    ...
    Handle<const ITexture> get_texture(
        ...
        if (!(expression_constant && aliased_type))
            return null;
    ...
    std::shared_ptr<Texture> recursive_texture_call(const IExpression * exp) {

        auto expression_factory = handle_of(
            state.factory->create_expression_factory(state.transaction.get()));

        if (exp->get_kind() == IExpression::EK_CALL) {

            std::string call_name = reinterpret_cast<const IExpression_call*>(exp)->get_call();            

            auto function_call = handle_of(
                state.transaction->access<IFunction_call>(call_name.c_str())
            );
            ...
            if (function_call) {
                ...
                logger().trace("\tCall name is {} param count: {}", call_name , function_call->get_parameter_count());
                ...
                for (size_t i = 0; i < function_call->get_parameter_count(); ++i)
                {
                    ...
                    const std::string param_text = handle_of(expression_factory->dump(param_expression.get(), 0, 1))->get_c_str();
                    ...
                    case IExpression::EK_CONSTANT:
                    {
                        ...
                        if (tex) {
                            ...
                            auto image = handle_of(state.transaction->access<IImage>(tex->get_image()));
                            if (!image)
                                break;
                            ...
                            return std::make_shared<Texture>(filename, p);
    ...
    std::shared_ptr<Material> load_adobe_standard_single(const IModule* module, int index) {
        ...
        auto material_definition = handle_of(
            state.transaction->access<IMaterial_definition>(
                module->get_material(index)
                )
        );
        ...
        auto value_factory = handle_of(
            state.factory->create_value_factory(state.transaction.get())
        );
        ...
        for (mi::Size index = 0; index < count; index++) {
            ...
            if (kind == IExpression::EK_CONSTANT) {
                ...
                break;
                default:
                    logger().warn("Value {} not supported", name );
                };
    ...
    MDL::Library load_adobe_standard(const std::string& base_dir, const std::string& module_name) {

        MDL::Library result;
        
        if (state.compiler->add_module_path(base_dir.c_str()) != 0)
        ...
        {
            if (state.compiler->load_module(state.transaction.get(), module_name.c_str(), context.get()) < 0) {
...
private:
    ...
    INeuray* load_ineuray(const char* filename = nullptr)
    {
        ...
        if (!neuray)
        {
            Handle<IVersion> version(
                mi_factory<IVersion>(symbol));
            if (!version)
...
{
    if (!m_instance)
        m_instance = std::make_unique<MDL>();

    return *m_instance;
}

MDL::Library MDL::load_materials(const std::string& base_dir, const std::string& module_name)
{
...
MDL::Library MDL::load_materials(
    const fs::path& LGUI_UNUSED(base_dir), const std::string& LGUI_UNUSED(module_name))
{
    return {};
}
