...
void KeybindsUI::draw()
{
    auto& k = get_viewer()->get_keybinds();
    auto& mapping = k.get();
...

    {
        ...
        if (button_icon(has_camera_scheme(k, DefaultCameraScheme::DIMENSION),
                "Dimension",
                "",
                "",
                true,
        ...
        ImGui::SameLine();
        if (button_icon(has_camera_scheme(k, DefaultCameraScheme::MAYA),
                "Maya",
                "",
                "",
                true,
        ...
        ImGui::SameLine();
        if (button_icon(has_camera_scheme(k, DefaultCameraScheme::BLENDER),
                "Blender",
                "",
                "",
                true,
        ...
        ImGui::SameLine();
        if (button_icon(has_camera_scheme(k, DefaultCameraScheme::SUBSTANCE),
                "Substance",
                "",
                "",
                true,
        ...
        if (selected >= 0 && selected < int(items.size())) {
            ...
            if (adding) {
                ...
                for (auto i = 0; i < Keybinds::keymap_size; i++) {
                    if (safe_cast<size_t>(tmp_keybind.modifier_count) == tmp_keybind.modifiers.max_size()) break;
                ...
                ImGui::TextColored(
                    ImVec4(ImColor(ImGui::Spectrum::GREEN500).Value), "Press Enter to Save");
