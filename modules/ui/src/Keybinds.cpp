...
void Keybinds::update(const std::string& context)
{
    ...
    const auto in_context = [&](const std::string& name) {
        return starts_with(name, context) ||
               starts_with(name, "global");
    };
    ...
    for (auto& it : m_mapping) {
        for (auto& keybind : it.second) {
            ...
            is_down &= (m_key_down_num == 1 + keybind.modifier_count);
            ...
            else if ((keybind.current_state == KeyState::DOWN ||
                         keybind.current_state == KeyState::PRESSED) &&
                     !is_down) {
                keybind.previous_state = keybind.current_state;
...
void Keybinds::add(
    const std::string& action, int button, const std::vector<int>& modifiers /*= {}*/)
{
...
bool Keybinds::has(
    const std::string& action, int button, const std::vector<int>& modifiers /*= {}*/) const
{
