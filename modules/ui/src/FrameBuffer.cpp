...
void FrameBuffer::set_color_attachement(unsigned int index,
    Resource<Texture> t,
    GLenum target /*= GL_TEXTURE_2D*/,
    int mipmap_level /*= 0*/)
{
    ...
    GL(glFramebufferTexture2D(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, target, id, mipmap_level));
    ...
    static GLuint attachments[8] = {GL_COLOR_ATTACHMENT0,
...
Resource<Texture> FrameBuffer::get_color_attachement(int index) const
{
    return m_color_attachments[index];
}

Resource<Texture> FrameBuffer::get_depth_attachment() const
{
