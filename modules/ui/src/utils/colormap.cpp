    ...
    int resolution /*= 256*/)
{
    ...
    auto p = Texture::Params::rgba16f();
    auto tex = std::make_shared<Texture>(p, resolution, 1, 0);

    // Generate data
    std::vector<Color> data(resolution);

    for (auto i = 0; i < resolution; i++) {
        const float val = (float(i) + 0.5f) / float(resolution);
        data[i] = generator(val);
    }

    tex->upload(reinterpret_cast<float*>(data.data()));
    return tex;
}
