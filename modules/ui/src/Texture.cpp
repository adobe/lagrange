...
Texture::Texture(const void* image_data,
    size_t size,
...
void Texture::resize(int width, int height /*= 0*/, int depth /*= 0*/, bool force /*= false*/)
{
    ...
    if(m_params.type == GL_TEXTURE_2D) {
        ...
        GL(glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE,
...
void Texture::set_common_params()
{
    ...
    if(!m_params.generate_mipmap && m_params.min_filter == GL_LINEAR_MIPMAP_LINEAR) {
        logger().warn("Mipmap disabled for tex id {} but using min filter GL_LINEAR_MIPMAP_LINEAR", m_id
                          );
    }
...
void Texture::load_data_from_image(const void* image_data, size_t size)
{
    int width, height, n;
    stbi_set_flip_vertically_on_load(true);
    unsigned char* data = stbi_load_from_memory(reinterpret_cast<const unsigned char*>(image_data),
 ...
 Texture::Transform::Transform(Eigen::Vector2f scale_ /*= Eigen::Vector2f(1.0f)*/,
