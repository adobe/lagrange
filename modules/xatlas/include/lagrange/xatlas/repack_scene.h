...
///
/// Repack all UV islands of a scene using xatlas.
///
/// Each mesh (when sharing UVs) or each instance (when not sharing) is repacked independently.
///
/// @param[in]  scene             Scene whose meshes already have an indexed UV attribute.
/// @param[in]  repack_options    Per-mesh repack options.
...
///
/// @return     A new scene with the same instance topology and repacked UV attributes.
///
template <typename Scalar, typename Index>
LA_XATLAS_API scene::SimpleScene<Scalar, Index, 3> repack_scene(
