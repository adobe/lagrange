...
SurfaceMesh<Scalar, Index> repack_mesh(
    ...
    const std::atomic_bool* cancel)
{
    if (mesh.get_num_facets() == 0) {
        logger().warn("lagrange::xatlas::repack_mesh: input mesh is empty; returning unchanged");
        return mesh;
    }
    ...
    engine.add_uv_mesh(mesh, options);
    engine.pack_charts(options.packing);
...
#define LA_X_repack_mesh(_, S, I)                       \
    template SurfaceMesh<S, I> repack_mesh<S, I>(       \
        const SurfaceMesh<S, I>&,                       \
        const RepackOptions&,                           \
        std::function<void(const std::string&, float)>, \
        const std::atomic_bool*);
LA_SURFACE_MESH_X(repack_mesh, 0)
#undef LA_X_repack_mesh
