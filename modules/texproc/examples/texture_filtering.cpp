...
int main(int argc, char** argv)
{
    ...
    app.add_option("--value-weight", filtering_options.value_weight, "Value interpolation weight.");
    app.add_option(
        "--gradient-weight",
        filtering_options.gradient_weight,
        "Gradient interpolation weight.");
    app.add_option(
        "--gradient-scale",
        filtering_options.gradient_scale,
        "Gradient scale factor (0 for smoothing, [2, 10] for sharpening).");
    app.add_option(
        "--quadrature",
    ...
    lagrange::texproc::texture_filtering(mesh, image.to_mdspan(), filtering_options);
