...
TEST_CASE("ThreadPool", "[texproc][thread]")
{
    std::atomic_bool is_thread_index_valid = true;
    std::atomic_bool is_loop_index_valid = true;
    unsigned int num_threads = lagrange::texproc::threadpool::ThreadPool::NumThreads();
    REQUIRE(num_threads > 0);
    lagrange::texproc::threadpool::ThreadPool::ParallelFor(0, 1000, [&](int tid, size_t i) {
