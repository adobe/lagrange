...
///
/// Unproject a vector of rendered images into per-camera (texture, weight) pairs.
///
/// @param[in]  mesh                  Input mesh with UVs.
/// @param[in]  base_texture          Optional base texture to seed the output with.
/// @param[in]  cameras               View and projection transforms for each camera.
/// @param[in]  renders               Rendered color images, one per camera.
    ...
    const std::optional<float> base_confidence)
{
    ...
    if (base_confidence.has_value() && base_confidence.value() == 0) {
        if (base_texture.has_value()) {
            lagrange::logger().warn("Base confidence is 0, ignoring provided base texture.");
        }
    } else {
        if (base_texture.has_value()) {
            ...
            if (base_confidence.has_value()) {
                lagrange::logger().warn(
                    "No base texture was provided. Ignoring user-provided base confidence: {}",
...
///
/// Overload of rasterize_textures_from_renders that takes a scene as input. The mesh, base texture
/// (if any), and cameras are extracted from the scene and forwarded to the primary overload. An
/// optional base texture override takes precedence over any texture already present in the scene.
///
template <typename Scalar, typename Index>
std::vector<std::pair<Array3Df, Array3Df>> rasterize_textures_from_renders(
    const lagrange::scene::Scene<Scalar, Index>& scene,
    std::optional<Array3Df> base_texture_override,
    const std::vector<ConstView3Df>& renders,
    const std::optional<size_t> tex_width,
    const std::optional<size_t> tex_height,
    const float low_confidence_ratio,
    const std::optional<float> base_confidence)
{
    ...
    auto cameras = scene::internal::camera_transforms_from_scene(scene);
    lagrange::logger().info("Found {} cameras in the input scene", cameras.size());

    if (base_texture_override.has_value()) {
        if (base_texture.has_value()) {
            lagrange::logger().warn(
                "Input scene already contains a base texture. Overriding with user-provided "
                "texture.");
        }
        base_texture = std::move(base_texture_override);
    }
