...
TEST_CASE("save_scene", "[io]")
{
    ...
    SECTION("OBJ - Stream Export Exception")
    {
        ...
        REQUIRE_THROWS_AS(
            io::save_scene_obj(ss, scene, options),
            std::runtime_error
        );
    }
    ...
    SECTION("OBJ - File Export With Materials")
    {
        ...
        REQUIRE(mtl_content.find("newmtl") != std::string::npos); // Should have material definitions
        ...
        fs::path base_color_texture = testing::get_test_output_path("test_save_scene/Avocado_baseColor.png");
        fs::path normal_texture = testing::get_test_output_path("test_save_scene/Avocado_normal.png");
