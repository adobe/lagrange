...
//
// So in the functions below we call `unify_index_buffer`. This can affect your vertex indexing,
...
void save_gltf(const fs::path& filename, const tinygltf::Model& model, const SaveOptions& options) {
    ...
    loader.WriteGltfSceneToFile(
        ...
        embed_buffers,
        pretty_print,
        options.encoding == FileEncoding::Binary);
}
...
{
    constexpr size_t buffer_max_size = ((size_t)2 << 32) - 1; // 2^32 - 1
    ...
    if (byte_offset + byte_length > buffer_max_size) {
        // a single data block cannot exceed the maximum buffer size
        la_runtime_assert(byte_length < buffer_max_size);
        model.buffers.push_back(tinygltf::Buffer());
        buffer = model.buffers.back();
        byte_offset = 0;
    }
    buffer.data.resize(byte_offset + byte_length);
    std::memcpy(&buffer.data.at(byte_offset), data.begin(), byte_length);
    ...
    const SaveOptions& options)
{
    ...
    for (Index i = 0; i < lscene.get_num_meshes(); ++i) {
        // There is a paragraph about this step at the beginning of this file.
        // TODO: maybe make this optional
        auto lmesh = unify_index_buffer(lscene.get_mesh(i));
        ...
        { // positions
            ...
            span<const float> data = get_attribute_as<Scalar, float>(lmesh.get_vertex_to_position().get_all(), tmp);
        ...
        { // face indices
            ...
            span<const uint32_t> data = get_attribute_as<Index, uint32_t>(lmesh.get_corner_to_vertex().get_all(), tmp);
        ...
        { // other attributes
            ...
            seq_foreach_named_attribute_read(lmesh, [&](std::string_view name, auto&& attr) {
                ...
                if constexpr(std::is_same_v<ValueType, char>) {
                    ...
                    logger().warn("Skipping attribute {}: unsupported type", name);
                    return;
                }
                ...
                if constexpr(std::is_same_v<ValueType, double>) {
                    ...
                    span<const ValueType> data = get_attribute_as<ValueType, ValueType>(values.get_all(), tmp);
                    std::tie(buffer_index, byte_offset, byte_length) = write_to_buffer<ValueType>(model, data);
                }
                ...
                std::transform(
                    name.begin(),
                    name.end(),
                    name_uppercase.begin(),
                    [](unsigned char c) { return std::toupper(c); });
                if (attr.get_usage() == AttributeUsage::Normal) {
                    if (found_normal) {
                        name_uppercase = "_" + name_uppercase;
                        logger().warn("Found multiple attributes for normal, saving {} as {}", name, name_uppercase);
                    } else {
                        found_normal = true;
                    ...
                    if (found_tangent) {
                        name_uppercase = "_" + name_uppercase;
                        logger().warn("Found multiple attributes for tangent, saving {} as {}", name, name_uppercase);
                    } else {
                        found_tangent = true;
                    ...
                    // Custom attributes are allowed ONLY if they start with a leading understore, e.g. _TEMPERATURE.
                    // Custom attribute MUST NOT use unsigned int type.
