...
class TestTorus:
    ...
    def test_partial_torus(self):
        mesh = lagrange.primitive.generate_torus(
            triangulate=True, start_sweep_angle=0, end_sweep_angle=math.radians(90)
        )
        assert lagrange.is_closed(mesh)
        assert lagrange.is_oriented(mesh)
        assert lagrange.is_manifold(mesh)
